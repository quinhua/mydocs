...
> 宿主机ping不通虚拟机,虚拟机可以ping通宿主机

```bash
# 打开虚拟机的`虚拟网络编辑器`,进入`VMnet8`中查看`net设置`
网络: vmnet8
子网IP: 192.168.199.0
子网掩码: 255.255.255.0
网关IP(G): 192.168.199.2

# 打开宿主机`更改网络适配器`,进入`VMnet8`的属性中,点击`Internet协议4(TCP/IPv4)`的属性中,
# 将自动获取IP地址换为使用下面的IP地址
子网IP: 192.168.199.(随便)
子网掩码: 255.255.255.0
网关IP(G): 192.168.199.2
```
